...
namespace FunctionalTests.Base
{
    ...
    public class TestFixture
    {
        ...
        public Task ExecuteDbContextAsync(Func<AppDbContext, Task> action) => ExecuteScopeAsync(sp => action(sp.GetService<AppDbContext>()));

        public Task<T> ExecuteDbContextAsync<T>(Func<AppDbContext, Task<T>> action) => ExecuteScopeAsync(sp => action(sp.GetService<AppDbContext>()));
        ...
        public Order CreateTestOrder()
        {
            ...
            var expenseFaker = new Faker<Expense>()
                              ...
                              .RuleFor(x => x.Product, f => (Product)f.Random.Int(0, 2));

            var timeRegistrationFaker = new Faker<TimeRegistration>()
                                       .RuleFor(x => x.Week, f =>
                                       {
                                           var date = f.Date.Soon(1);
                                           return $"{ISOWeek.GetYear(date)}{ISOWeek.GetWeekOfYear(date)}";
                                       }).RuleFor(x => x.DateTime, _ => DateTime.UtcNow);

            var order = orderFaker.Generate();
            var worker = workerFaker.Generate();
            var expenses = Enumerable.Range(0, 10).Select(_ => expenseFaker.Generate()).ToArray();
            var timeRegistrations = Enumerable.Range(0, 10).Select(_ => timeRegistrationFaker.Generate()).ToArray();

            timeRegistrations.ForAll(x =>
            {
                x.Worker = worker;
                x.Order = order;
            });
